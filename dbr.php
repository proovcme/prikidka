<?php 
  $pageTitle = "Барабан-Буфер-Веревка (DBR) в проектировании | Метод ТОС"; 
  $pageDescription = "Анимация и разбор механизма Барабан-Буфер-Веревка (DBR) для проектных бюро. Как управлять потоком задач через самое узкое место, защищать дедлайны буфером и синхронизировать исполнителей с помощью каната.";
  $pageKeywords = "DBR, Барабан буфер веревка, теория ограничений, ТОС проектирование, управление производством, метод критической цепи, CCPM, балансировка ресурсов";
  $canonicalUrl = "https://toc.chernetchenko.pro/dbr";
  $siteId = 'toc';
  include 'header.php'; 
?>
...
<style>
        ...
        .station-drum .station-box     { background: var(--c-toc); border: 2px solid var(--c-rag); }
        .station-overload .station-box { background: var(--c-ai); animation: pulse-red 1s infinite; }
        ...
        .work-item.blocked { background: var(--c-ai); border-color: #901818; }
        .work-item.processed { background: var(--c-toc); border-color: #1d5a28; }
        ...
        .anim-btn-play  { background: var(--c-toc);   color: #fff; }
        .anim-btn-drum  { background: var(--c-rag);  color: #fff; }
        .anim-btn-rope  { background: var(--c-toc);   color: #fff; }
        .anim-btn-reset { background: #e8e4dc; color: var(--c-toc); border: 1px solid var(--border); }
        ...
        .gantt-bar {
            ...
            color: #fff;
            white-space: nowrap;
            transition: width 0.4s cubic-bezier(0.4, 0, 0.2, 1), left 0.4s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.4s ease;
        }

        .bar-task    { background: #4a6888; }
        .bar-sleep   { background: #8a9aaa; }
        .bar-panic   { background: var(--c-ai); }
        .bar-buffer  { background: var(--c-rag); }
        .bar-relay   { background: var(--c-toc); }
        .bar-done    { background: var(--c-toc); }
        ...
        .metric-val.bad  { color: var(--c-ai); }
        .metric-val.good { color: var(--c-toc); }
        .metric-val.warn { color: var(--c-rag); }
...
.btn { padding: 0.85rem 1.4rem; border-radius: var(--r-md); font-family: var(--font-ui); font-size: 0.8rem; font-weight: 800; text-transform: uppercase; text-decoration: none; transition: all 0.3s cubic-bezier(0.2, 0.8, 0.2, 1); text-align: center; letter-spacing: 0.05em; display: inline-block; cursor: pointer; }
...
@media (max-width: 600px) { .container { padding: 20px 16px 48px !important; } }</style>
    <article itemscope itemtype="https://schema.org/TechArticle">
        <header style="padding: 4.5rem 0 3rem; border-bottom: 2px solid var(--text-main); margin-bottom: 2rem;">
            <h1 itemprop="headline" style="font-family: var(--font-ui); font-size: clamp(2rem, 6vw, 3.5rem); font-weight: 900; line-height: 1.1; margin-bottom: 1.2rem; color: var(--text-main);">Механика DBR: Барабан, Буфер, Канат</h1>
            <p itemprop="description" style="font-size: 1.05rem; color: var(--text-muted); max-width: 680px; line-height: 1.7;">Интерактивная модель управления производственным и проектным потоком по Теории Ограничений. Как перестать балансировать мощности и начать управлять потоком.</p>
        </header>

        <div class="content-block" style="padding-top:0;">
            <h2 class="section-title">Почему балансировка мощностей — это миф</h2>
            <p style="font-size: 1.05rem; line-height: 1.8; color: var(--text-muted); margin-bottom: 1.5rem;">
                В классическом управлении считается, что для максимальной эффективности каждый участник процесса должен быть загружен на 100%. Это приводит к попыткам «сбалансировать мощности» — выровнять пропускную способность всех подразделений. Однако в реальности статистические колебания (кто-то заболел, программа зависла, заказчик долго думал) накапливаются, и идеально сбалансированная система моментально теряет управляемость. Очереди растут, сроки срываются.
            </p>
            <p style="font-size: 1.05rem; line-height: 1.8; color: var(--text-muted); margin-bottom: 1.5rem;">
                Теория Ограничений Голдратта предлагает другой подход: <b>балансировать нужно не мощности, а поток</b>. В любой системе всегда есть одно самое узкое место — ограничение. Именно оно диктует темп работы всего проектного института. Чтобы система работала предсказуемо, был разработан механизм Барабан-Буфер-Веревка (Drum-Buffer-Rope).
            </p>
        </div>

  <article itemscope itemtype="https://schema.org/TechArticle">
    <header class="article-hero"><h1 itemprop="headline">Барабан — Буфер — Канат</h1><p class="hero-desc" itemprop="description"></p></header>
        ...
        <div class="anim-stage">
            ...
            <div class="legend-row">
                ...
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-toc);outline:2px solid var(--c-rag)"></div>Барабан (узкое место)</div>
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-ai)"></div>Перегружена</div>
                <div class="legend-item"><div class="legend-dot" style="background:#7a9ab8"></div>Задание в системе</div>
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-toc)"></div>Задание выполнено</div>
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-rag)"></div>Канат (сигнал запуска)</div>
            </div>
        </div>

        <!-- Пояснение DBR -->
        <div class="intro-block card card-left-navy" style="margin-bottom: 36px;">
            <h2 style="color:var(--c-toc); margin-top:0; font-size:18px;">Что ты видишь</h2>
        ...
        <div class="anim-stage">
            ...
            <div class="legend-row">
                ...
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-ai)"></div>Аврал (выход за дедлайн)</div>
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-toc)"></div>Работа по CCPM (эстафета)</div>
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-rag)"></div>Общий буфер проекта</div>
                <div class="legend-item"><div class="legend-dot" style="background:var(--c-toc)"></div>Завершено в срок</div>
            </div>
        </div>
 ...
 (_)-(_)-(_)
    ...
    function dbrRender() {
        ...
        bufFill.style.background = pct > 60 ? 'var(--c-toc)' : pct > 30 ? 'var(--c-rag)' : 'var(--c-ai)';
    ...
    function ccpmRender() {
        ...
        if (d > CC_BUFFER_START) {
            ...
            bufLbl.style.cssText = `position:absolute;left:${dayToX(CC_BUFFER_START) + 4}px;top:176px;font-size:10px;font-weight:700;color:var(--c-rag);z-index:2`;
        ...
        if (d >= CC_ACTUAL) {
            ...
            dline.style.cssText = `position:absolute;left:${dayToX(CC_ACTUAL)}px;top:155px;bottom:20px;width:2px;background:var(--c-toc);z-index:3`;
            canvas.appendChild(dline);
            const dlbl = document.createElement('div');
            dlbl.style.cssText = `position:absolute;left:${dayToX(CC_ACTUAL) + 4}px;top:158px;font-size:9px;font-weight:700;color:var(--c-toc)`;
