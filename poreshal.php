...
/* Шапка-статус */
.p77-hero { background: var(--c-toc); color: #fff; padding: 28px 30px 22px; border-bottom: 3px solid var(--c-rag); margin-bottom: 28px; display: flex; justify-content: space-between; align-items: center; }
...
.p77-section-head { background: var(--bg-tertiary); border-bottom: 1px solid var(--border); padding: 11px 18px; display: flex; align-items: center; gap: 10px; }
.p77-section-num { width: 22px; height: 22px; background: var(--c-toc); color: #fff; border-radius: 50%; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
.p77-section-title { font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: var(--c-toc); }
.p77-section-body { padding: 20px 18px; display: grid; grid-template-columns: repeat(auto-fit, minmax(240px, 1fr)); gap: 14px; }

/* Кнопка */
.p77-btn { display: block; width: 100%; background: var(--c-ai); color: #fff; border: none; border-radius: 4px; padding: 22px; font-size: 22px; font-weight: 900; letter-spacing: 3px; text-transform: uppercase; cursor: pointer; margin-top: 28px; transition: transform 0.08s, box-shadow 0.08s; box-shadow: 0 6px 20px rgba(176,48,48,0.4); position: relative; overflow: hidden; }
...
.stamp-reject { color: var(--c-ai); border-color: var(--c-ai); }
.stamp-warn   { color: #c47a00; border-color: #c47a00; }
.stamp-ok     { color: var(--c-toc); border-color: var(--c-toc); }
...
.p77-terms { background: #f8f8f6; border-left: 4px solid var(--c-toc); padding: 18px 20px; font-size: 13.5px; color: #333; line-height: 1.7; margin-bottom: 28px; position: relative; z-index: 1; }
...
<div class="p77-wrap">
    ...
    <div id="p77-doc" class="p77-doc">
        ...
        <div id="p77-risk-section" style="position:relative;z-index:1;margin-bottom:20px;">
            ...
            <div class="p77-risk-bar"><div class="p77-risk-fill" id="p77-risk-fill" style="width:0%;background:var(--c-toc);"></div></div>
        ...
        <div class="p77-sign">
            ...
            <div style="text-align:right;">
                <div id="p77-verdict-box" style="border:2px solid var(--c-toc);padding:12px 16px;font-size:12px;font-weight:700;color:var(--c-toc);transform:rotate(4deg);line-height:1.5;">
...
function p77Calculate() {
    ...
    document.getElementById('p77-risk-fill').style.background = riskScore < 7 ? 'var(--c-toc)' : riskScore < 12 ? 'var(--c-rag)' : 'var(--c-ai)';
    ...
    if (status === 'reject') {
        ...
        vb.style.borderColor = 'var(--c-ai)'; vb.style.color = 'var(--c-ai)';
    ...
    if (isReject) {
        document.getElementById('p77-intro').innerHTML = `${introBase}<br><br><b>Результат:</b> Проект классифицирован как <b style="color:var(--c-ai)">НЕЦЕЛЕСООБРАЗНЫЙ К РАССМОТРЕНИЮ</b>.<br><br><b>Выявленные стоп-факторы:</b><ul style="margin:10px 0 18px 22px;line-height:1.8;">${stopReasons.map(r => `<li>${r}</li>`).join('')}</ul>Согласно п. 1.4 Регламента ДОПУСК-2077, объекты с данными параметрами исключаются из воронки продаж без права на обжалование. Департамент не распыляет ресурсы на нестратегические активы.`;
        document.getElementById('p77-tbody').innerHTML = `<tr><td colspan="6" style="text-align:center;padding:40px;font-weight:bold;color:var(--c-ai);font-size:15px;">РАСЧЁТ СТОИМОСТИ НЕ ПРОИЗВОДИЛСЯ.<br>ПРОЕКТ ИСКЛЮЧЁН ИЗ ВОРОНКИ.</td></tr>`;
