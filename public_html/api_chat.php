...
// ======================================================================================
$apiKey = 'ТВОЙ_OPENROUTER_API_КЛЮЧ'; // Вставь свой ключ sk-or-v1-...
...
// ➔ СЦЕНАРИЙ 1: Запрос из Ганта (Аудит графика и Рисков)
else if (isset($data['context'])) {
    $userMessage = $data['message'] ?? 'Полный аудит.';
    $contextData = $data['context'];
    
    $systemPrompt = "Ты Иона, жесткий и прагматичный эксперт по управлению проектами на базе Теории Ограничений (ТОС) Элияху Голдратта. 
    Твоя задача - проанализировать переданный контекст ПИР.
    Отвечай на русском языке, кратко, структурировано, без воды.
    Найди 'узкое горлышко', оцени загрузку, риски, буфер. Дай 2-3 жесткие рекомендации по ТОС.";
